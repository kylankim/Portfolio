...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(self.ant, QueenAnt) and self.ant.real:
                return
            if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                if isinstance(self.ant.ant, QueenAnt) and self.ant.ant.real:
                    return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if current_place.bees != [] and self.max_range != 3 and self.min_range == 0:
                return random_or_none(current_place.bees)
            if self.min_range != 0:
                if dist >= 5 and current_place.bees != []:
                    return random_or_none(current_place.bees)
            if self.max_range == 3:
                if dist <= 3 and current_place.bees != []:
                    return random_or_none(current_place.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.real == False:
            ...
            while check:
                if check.ant != None and check.ant.container:
                    if check.ant.ant:
                        if check.ant.ant not in self.affected:
                            check.ant.ant.damage *= 2
                            self.affected.append(check.ant.ant)
                if check.ant:
                    if check.ant not in self.affected:
                        check.ant.damage *= 2
                        self.affected.append(check.ant)
                check = check.exit
