...
def closer_city(lat, lon, city1, city2):
    ...
    current_location = make_city('CL', lat, lon)
    
    x1 = get_lon(city1)
    x2 = get_lon(city2)
    x3 = get_lon(current_location)
    y1 = get_lat(city1)
    y2 = get_lat(city2)
    y3 = get_lat(current_location)
    first = (float)(sqrt((x1 - x3)**2 + (y1 - y3)**2))
    second = (float)(sqrt((x2 - x3)**2 + (y2 - y3)**2))
    if first > second:
