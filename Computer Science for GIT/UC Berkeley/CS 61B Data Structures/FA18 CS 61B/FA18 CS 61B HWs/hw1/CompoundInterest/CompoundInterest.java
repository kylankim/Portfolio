...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double temp_future = futureValue(presentValue, rate, targetYear);
        double r_rate = 1 - (inflationRate/100);
        for(int i = 0; i < numYears(targetYear); i++){
            temp_future *= r_rate;
        }
        return temp_future;
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double r_rate = 1 + (rate/100);
        double r2_rate = 1 - (inflationRate/100);
        double total_sum = perYear;
        for(int i = 0; i < numYears(targetYear); i++){
            total_sum *= r_rate;
            total_sum *= r2_rate;
