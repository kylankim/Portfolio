...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                _solnNumToPlace[_solution[i][j] - 1] = pl(i, j);
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int direct = arrowDirection(i, j);
                if (_solution[i][j] == 1) {
                    _board[i][j] = new Sq(i, j, 1, true, direct, -1);
                } else if (_solution[i][j] == last) {
                    _board[i][j] = new Sq(i, j, last, true, direct, -1);
                } else {
                    _board[i][j] = new Sq(i, j, 0, false, direct, 0);
                }
    ...
    /** Initializes a deep copy of MODEL. */
    Model(Model model) {
    ...
    private int arrowDirection(int x, int y) {
        ...
        Place sol = _solnNumToPlace[seq0];
        return Place.dirOf(x, y, sol.x, sol.y);
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (Place.dirOf(this.x, this.y, s1.x, s1.y) == 0) {
                ...
                return _sequenceNum == s1.sequenceNum() - 1;
            }
        ...
        boolean connect(Sq s1) {
            ...
            _successor._head = _head;
            Sq next = this;
            if (_sequenceNum != 0) {
                while (next._successor != null) {
                    next._successor._sequenceNum = next._sequenceNum + 1;
            ...
            if (s1._sequenceNum != 0) {
                while (back._predecessor != null) {
                    back._predecessor._sequenceNum = back._sequenceNum - 1;
                    back = back._predecessor;
                }
            }
            if (s1._sequenceNum == 0 && this._sequenceNum != 0) {
                releaseGroup(s1._group);
                this._head._group = s1._head._group;
            } else if (this._sequenceNum == 0 && s1._sequenceNum != 0) {
                releaseGroup(_group);
                s1._head._group = this._head._group;
            } else if (this._sequenceNum == 0 && s1._sequenceNum == 0) {
                _head._group = joinGroups(_head._group, s1._group);
            }
            return true;
        }
