...
(define (enumerate s)
  ; BEGIN PROBLEM 17
  (define (index_element index s)
    (if (null? s) nil
      (cons (cons index (cons (car s) nil)) (index_element (+ index 1) (cdr s)))
      ))
      (index_element 0 s)
      )
  ; END PROBLEM 17
...
(define (list-change total denoms)
  ; BEGIN PROBLEM 18
  (cond ((= total 0) (cons nil nil))((or (< total 0) (null? denoms)) nil)
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           ; BEGIN PROBLEM 19
           (cons (cons 'lambda 
            (cons (let-to-lambda (car (zip values))) 
              (let-to-lambda body)))
