...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        if (start.sequenceNum() == model.height() * model.width()) {
            return null;
        }
        for (Place a : start.successors()) {
            if (start.connectable(model.get(a))) {
                if (start.sequenceNum() != 0 && model.get(a).sequenceNum()
                        == start.sequenceNum() + 1) {
                    return model.get(a.x, a.y);
                } else {
                    A.add(a);
                }
            }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        for (Place a : end.predecessors()) {
            if (model.get(a.x, a.y).connectable(end)
                    && model.get(a).sequenceNum() == 0
                    && end.sequenceNum() == 0) {
                A.add(a);
            } else if (model.get(a.x, a.y).connectable(end)) {
                B.add(a);
            }
        }
