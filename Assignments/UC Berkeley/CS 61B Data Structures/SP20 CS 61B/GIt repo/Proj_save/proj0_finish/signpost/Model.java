...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _solnNumToPlace = new Sq[last + 1];
        boardMaker();
        for (int k = 1; k < last; k++) {
            if (solnNumToPlace(k) == null) {
                throw new IllegalArgumentException();
            }
        }
        for (int q = 0; q < _width; q++) {
            for (int w = 0; w < _height; w++) {
                for (int e = 0; e < _width; e++) {
                    for (int r = 0; r < _height; r++) {
                        if (_board[e][r]._successors.contains(pl(q, w))) {
                            _board[q][w]._predecessors.add(pl(e, r));
                        }
                    }
                }
            }
    ...
    /** method to assign new squares to the board. */
    private void boardMaker() {
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int sol = _solution[i][j];
                if (sol == _width * _height) {
                    _board[i][j] = new Sq(i, j, sol, true, 0, 0);
                } else {
                    for (int a = 0; a < _width; a++) {
                        for (int b = 0; b < _height; b++) {
                            if (_solution[a][b] == sol + 1) {
                                if (sol == 1) {
                                    _board[i][j] = new Sq(i, j, sol, true,
                                            dirOf(i, j, a, b), 0);
                                    break;
                                } else {
                ...
                _solnNumToPlace[sol] = _board[i][j];
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                Sq sample = model.get(i, j);
                _board[i][j] = new Sq(sample.x, sample.y, sample._sequenceNum,
                        sample._hasFixedNum, sample.direction(), sample._group);
                _board[i][j]._predecessors = new PlaceList();
        ...
        for (Sq a : _allSquares) {
            Sq sample = model.get(a.x, a.y);
            if (sample.successor() != null) {
                a._successor = _board[sample._successor.x][sample._successor.y];
            }
            if (sample.predecessor() != null) {
                a._predecessor
                        = _board[sample._predecessor.x][sample._predecessor.y];
            }
            if (sample.head() != null) {
                a._head = _board[sample.head().x][sample.head().y];
            }
            for (Place pla : sample.successors()) {
                a._successors.add(pla);
            }
            for (Place pla : sample.predecessors()) {
                a._predecessors.add(pla);
            }
        }
    ...
    boolean autoconnect() {
        ...
        for (Sq a : _allSquares) {
            for (Sq b : _allSquares) {
                if (a.sequenceNum() != 0 && b.sequenceNum()
                        != 0 && a.connectable(b)) {
                    a.connect(b);
                    checker = true;
                }
     ...
     */
    protected int arrowDirection(int x, int y) {
    ...
    final class Sq {
        ...
        Sq(int x0, int y0, int sequenceNum, boolean fixed, int dir,
            int group) {
            x = x0;
            y = y0;
            pl = pl(x, y);
            ...
            if (sequenceNum() != 0) {
                if (s1.sequenceNum() == 0) {
                    ...
                    for (Sq tmp1 = s1; tmp1 != null; tmp1 = tmp1._successor) {
                        tmp1._sequenceNum
                                = (tmp1.predecessor().sequenceNum() + 1);
                        tmp1._group = 0;
                    }
                }
                ...
                for (Sq tmp2 = this; tmp2 != null; tmp2 = tmp2._predecessor) {
                    tmp2._sequenceNum = (tmp2.successor().sequenceNum() - 1);
                    tmp2._group = 0;
                }
            } else {
        ...
        /** disconnect when only there exist no group exists.
         * @param next next
         * */
        void disconnectIntoSquares(Sq next) {
            next._head = next;
            if (this.hasFixedNum() && next.hasFixedNum()) {
                return;
            } else if (this.hasFixedNum() && !next.hasFixedNum()) {
                next._group = -1;
                next._sequenceNum = 0;
            } else if (next.hasFixedNum()) {
                _sequenceNum = 0;
                _group = -1;
            } else if (!hasFixedNum() && !next.hasFixedNum()) {
                next._sequenceNum = _sequenceNum = 0;
                releaseGroup(group());
                next._group = _group = -1;
            }
        }

        /** Disconnect when only this is left alone after disconnect.
         *
         * @param fixedChecker fixedChecker
         * @param next next
         * @return if the chage on the fixedChecker has been made
         * */
        boolean disconnectThis(Sq next, boolean fixedChecker) {
        ...
        /** Disconnect when only this is left alone after disconnect.
         * @param next next
         * @param fixedChecker fixedChecer
         * @return if the chage on the fixedChecker has been made
         *  */
        boolean disconnectNext(Sq next, boolean fixedChecker) {
            next._head = next;
            for (Sq a = this; a != null && !fixedChecker;
                 a = a.predecessor()) {
                fixedChecker = a.hasFixedNum();
            }
            if (next.hasFixedNum() && !fixedChecker) {
        ...
        void disconnect() {
            ...
            _unconnected += 1;
            boolean fixedChecker = false;
            this._successor = next._predecessor = null;
            if (predecessor() == null && next.successor() == null) {
                disconnectIntoSquares(next);
            } else if (predecessor() == null) {
                fixedChecker = disconnectThis(next, fixedChecker);
            } else if (next.successor() == null) {
                fixedChecker = disconnectNext(next, fixedChecker);
            } else {
                for (Sq a = next; a != null; a = a.successor()) {
                    if (a.hasFixedNum()) {
                        fixedChecker = true;
                    ...
                    a._head = next;
                } boolean tmpChecker = false;
                for (Sq b = this; b != null
                        && !tmpChecker; b = b.predecessor()) {
                    tmpChecker = b.hasFixedNum();
                }
                if (!fixedChecker) {
                    if (tmpChecker) {
                        for (Sq a = next; a != null; a = a.successor()) {
