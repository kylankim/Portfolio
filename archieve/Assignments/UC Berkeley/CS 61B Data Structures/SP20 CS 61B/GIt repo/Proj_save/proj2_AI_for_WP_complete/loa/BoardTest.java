...
//        assertEquals(tmp.toString(), new Board().toString());
        Board tmp2 = new Board(BOARD2, BP);
    ...
    public void testcopyForm() {
        ...
        b1.copyFrom(b2);
        for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {
            assertEquals("Contents of two boards are different",
                    b1.get(sq(i % 8, i / 8)), b2.get(sq(i % 8, i / 8)));
        }
