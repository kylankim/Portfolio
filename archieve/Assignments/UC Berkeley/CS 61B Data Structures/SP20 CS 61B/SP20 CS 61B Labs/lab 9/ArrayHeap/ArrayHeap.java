...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        ...
        if (node1 == null || node2 == null) {
            ...
            double priority1 = node1.priority();
            double priority2 = node2.priority();
    ...
    private void bubbleUp(int index) {
        if (index != 1) {
            int parentId = getParentOf(index);
            int smallerId = min(index, parentId);
    ...
    private void bubbleDown(int index) {
        int lChildId = getLeftOf(index);
        int rChildId = getRightOf(index);
        Node myNode = getNode(index);
        Node lChildNode = getNode(lChildId);
        Node rChildNode = getNode(rChildId);
        int smallerId = 0;

        if (lChildId == 0 && rChildId == 0) {
        } else if (lChildId == 0 && rChildId != 0) {
            if (myNode.priority() > rChildNode.priority()) {
                swap(index, rChildId);
                bubbleDown(rChildId);
            }
        } else if (lChildId != 0 && rChildId == 0) {
            if (myNode.priority() > lChildNode.priority()) {
                swap(index, lChildId);
                bubbleDown(lChildId);
            }
        } else {
            smallerId = min(lChildId, rChildId);
            swap(index, smallerId);
            bubbleDown(smallerId);
        }
    }
